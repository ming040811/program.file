document.addEventListener('DOMContentLoaded', () => {
    ...
    // ... (기본 DOM 요소, storyData, selectedDecoId, toastTimer 등은 기존과 동일) ...
    ...
    async function syncStateToFirestore() {
        ...
        const decoListForMobile = storyData[currentScene].decorations.map(deco => {
            ...
            // ⭐ [수정] 렌더링 전/후를 모두 고려하여 너비/높이 값 참조
            const elemWidth = decoElement ? decoElement.offsetWidth : deco.width;
            const elemHeight = decoElement ? decoElement.offsetHeight : deco.height;

            return {
                id: deco.id,
                // 모바일 X (가로) = PC Y (세로) + (높이/2)
                x_mobile: (deco.y + elemHeight / 2) / canvasHeight,
                // 모바일 Y (세로) = PC X (가로) + (너비/2)
                y_mobile: (deco.x + elemWidth / 2) / canvasWidth
            };
        });
    ...
    // --- 아이템 선택 처리 함수 ---
    // (기존과 동일 - syncStateToFirestore() 호출 유지)
    function selectItem(id) {
        ...
        syncStateToFirestore(); // PC에서의 선택은 모바일로 동기화해야 함
    }
    ...
    function handleItemMove(id, mobileX, mobileY) {
        ...
        decoData.x = (mobileY * canvasWidth) - (elemWidth / 2);
        decoData.y = (mobileX * canvasHeight) - (elemHeight / 2);

        updateElementStyle(decoData);
        
        // ⭐ [핵심 수정] 느려짐/깜박임 방지
        // 모바일에서 온 명령을 다시 모바일로 되돌려 보내지 않습니다. (Echo 방지)
        // syncStateToFirestore(); // 제거
        updateThumbnail(currentScene); // 썸네일만 업데이트
    }
    ...
    function handleControllerControl(id, action, data) {
        ...
        if (action === 'rotate') {
            ...
            const element = document.getElementById(id);
            const elemWidth = element ? element.offsetWidth : decoData.width;
            const elemHeight = element ? element.offsetHeight : decoData.height;
            
            if (elemWidth * factor > 20 && elemHeight * factor > 20) {
                const deltaWidth = (elemWidth * factor) - elemWidth;
                const deltaHeight = (elemHeight * factor) - elemHeight;
            ...
            if (index > -1) {
                ...
                // ⭐ [수정] 삭제는 '리스트'가 변경되므로 반드시 동기화(sync)해야 함
                if (selectedDecoId === id) {
                    selectItem(null); // selectItem()이 syncStateToFirestore()를 호출
                } else {
                    syncStateToFirestore(); // 리스트 변경을 모바일에 알려야 함
                }
    ...
    document.querySelectorAll('.asset-item[data-type="decoration"]').forEach(item => {
        item.addEventListener('click', () => {
            ...
            renderScene(currentScene); // 렌더링 (내부에서 sync 호출)
            selectItem(newDeco.id); // 아이템 선택 (내부에서 sync 호출)
        });
    });
    ...
    function renderScene(sceneNumber) {
        ...
        // ⭐ [수정] selectItem(null) 대신 현재 selectedDecoId로 호출
        selectItem(selectedDecoId); 
        
        setTimeout(() => updateThumbnail(sceneNumber), 50); 
        // syncStateToFirestore(); // selectItem이 호출하므로 중복
    }
    ...
    // (PC 조작 시에는 syncStateToFirestore()가 반드시 호출되어야 하므로, 이 함수는 수정 없음)
    function makeInteractive(element) {
        ...
        element.addEventListener('mousedown', (e) => {
            selectItem(element.id); // (내부에서 sync 호출)
            e.stopPropagation();
        });
        ...
        function closeDragElement() {
            ...
            updateThumbnail(currentScene);
            syncStateToFirestore(); // PC 조작 완료 후 동기화 (필수)
        }
        ...
        if (deleteButton) {
            deleteButton.addEventListener('click', (e) => {
                e.stopPropagation();
                handleControllerControl(element.id, 'delete'); // (내부에서 sync 호출)
            });
        }
    } // --- makeInteractive 끝 ---
    ...
    scenes.forEach(scene => {
        scene.addEventListener('click', () => {
            ...
            renderScene(currentScene); // (내부에서 sync 호출)
        });
    });
